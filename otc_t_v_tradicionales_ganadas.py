# -- coding: utf-8 --
#########################################################################################################
# NOMBRE: otc_t_v_tradicionales_ganadas.py										                        #
# DESCRIPCION:																							#
#  Lee la tabla v_tradicionales_ganadas_DAS de la base externa SQL Server y la carga a Hive				#
...
spark = SparkSession\
    .builder\
    .appName("OTC_T_V_TRADICIONALES_GANADAS")\
    .master("local")\
    .enableHiveSupport()\
    .getOrCreate()
...
query = "(SELECT item,codigo_caso,codbien,nomcli,ruccli,region,comercial,idc,jefecomercial,negocio,complejidad,tipo_requerimiento,numpuntos, "
query = query + "canaldas,servicio,medio,vser,vum,vequi,vnrc,vmrc,subsidio,fecha_entrada as fecha_ganada,nrc_actual,mrc_actual,caudal,codigo_das, "
query = query + "codigo_localidad,nombre_das,ruc_das,jeferegional,origen,fac_terminatx,numeroip,numeroventa,username,canal_comercial"
