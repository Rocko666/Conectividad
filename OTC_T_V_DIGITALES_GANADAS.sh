set -e
#########################################################################################################
# NOMBRE: OTC_T_V_DIGITALES_GANADAS.sh			     	      								            #
# DESCRIPCION:																							#
#   Shell que extrae la información de la tabla v_digitales_ganadas_DAS de SQLServer a Hive				#
...
VAL_DIA=`date '+%Y%m%d'` 
VAL_HORA=`date '+%H%M%S'` 
VAL_LOG=$VAL_RUTA/log/OTC_T_V_DIGITALES_GANADAS_$VAL_DIA$VAL_HORA.log
...
$VAL_RUTA/python/otc_t_v_digitales_ganadas.py \
--vUrl=$VAL_URL \
...
if [ $error_spark -eq 0 ];then
	echo "==== OK - La ejecucion del archivo spark otc_t_v_digitales_ganadas.py es EXITOSO ===="`date '+%H%M%S'` 2>&1 &>> $VAL_LOG
	else
	echo "==== ERROR: - En la ejecucion del archivo spark otc_t_v_digitales_ganadas.py ====" 2>&1 &>> $VAL_LOG
	exit 1
fi
...
echo "==== Finaliza extraccion tabla V_DIGITALES_GANADAS_DAS de SQLServer ===="`date '+%Y%m%d%H%M%S'` 2>&1 &>> $VAL_LOG